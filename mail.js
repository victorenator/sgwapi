...
import {POST, Client} from './client.js';
...
export class Mail {
     ...
     */
    send({to, cc, bcc, replyTo, from, subject, text, html, attachments, templateID, sections, headers, categories,
            customAgrs, sendAt, batchID, asm, ipPoolName, mainSettings, trackingSettings}) {
        assert.ok(to, 'recipients is required');
        assert.ok(Array.isArray(to), 'recipients must be array');
        assert.ok(from, 'sender email is required');
        assert.ok(subject, 'subject is required');
        assert.ok(text || html, 'body is required');
        if (cc) {
            assert.ok(Array.isArray(cc), 'CC recipients must be array');
        }
        if (bcc) {
            assert.ok(Array.isArray(bcc), 'BCC recipients must be array');
        }
